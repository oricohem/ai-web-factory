...
- Before any full website generation, landing page generation, launch kit site generation, front-door product experience generation, multi-page prototype generation, major website redesign, or high-stakes visual direction work, use `ai-full-site-builder-workflow` and `docs/full-site-builder-workflow-v1.md` as the mandatory orchestration layer. Future agents must not code directly from the user prompt; they must run the required engine order and produce architecture decision, internal production prompt, visual direction, section plan, launch kit output plan, build plan, and critique/checklist plan before implementation.
...
- Before high-fidelity website generation, premium landing-page implementation, customer-facing front-door work, launch-kit site implementation, or major visual redesigns, use `ai-external-ui-design-stack-integration` and `docs/external-ui-design-stack-integration-v1.md` as the required external design execution layer after architecture and launch planning. External stacks are optional execution support only and must not override business strategy, archetype selection, launch-kit logic, or trust/conversion decisions.
- **Pure Visual Execution Mode override (minimal):** When the user explicitly asks for pure visual execution, visual redesign, premium UI pass, North Star page, component recipe extraction, high-end landing page visual craft, or improving visual quality without changing strategy, temporarily suspend blocking emphasis on full-site orchestration artifacts, launch-kit documentation packaging, broad architecture rewrites, matrix/scoring expansion, and docs/prototype-first framing.
- In Pure Visual Execution Mode, active controllers are: (1) `high-end-visual-design`, (2) `design-taste-frontend`, (3) `ai-factory-taste-engine` anti-generic gates.
- In Pure Visual Execution Mode, strategy/architecture skills remain reference-only unless the user explicitly asks for strategy artifacts; do not create new architecture docs/matrices by default.
- In Pure Visual Execution Mode, do not let launch-kit packaging turn visual output into an internal spec; execute from concrete component composition, type scale, spacing, layout, responsive behavior, and interaction states.
- In Pure Visual Execution Mode, every major section must be implemented as a designed component, not explanatory documentation.
- Scope safety still applies: this override does not authorize production UI edits unless the user explicitly requests production work; it only changes the docs/prototype visual-craft role from prototyper/documenter to production designer.
...
- Enforce external-stack licensing and anti-copy rules before reusing external code, text, components, or patterns: verify license/attribution requirements, prefer principle extraction, and reject unclear-license copying.
